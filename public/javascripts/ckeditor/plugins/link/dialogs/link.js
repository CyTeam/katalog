...
function(a) {
    ...
    c = function() {
        ...
        D = C.getContentElement('target', 'popupFeatures'),
        E = C.getContentElement('target', 'linkTargetName'),
        F = this.getValue();
        if (!D || !E) return;
        D = D.getElement();
        D.hide();
    ...
    d = function() {
        var C = this.getDialog(),
        D = ['urlOptions', 'anchorOptions', 'emailOptions'],
        E = this.getValue(),
        F = C.definition.getContents('upload'),
        G = F && F.hidden;
        if (E == 'url') {
    ...
    o = /(?:^|,)([^=]+)=(\d+|yes|no)/gi,
    p = function(C, D) {
        var E = D && (D.getAttribute('_cke_saved_href') || D.getAttribute('href')) || '',
        F,
        G,
        H,
        I,
        J = {};
        if (F = E.match(e)) if (y == 'encode') E = E.replace(l,
        function(Z, aa, ab) {
            return 'mailto:' + String.fromCharCode.apply(String, aa.split(',')) + (ab && w(ab));
        });
        else if (y) E.replace(m,
        function(Z, aa, ab) {
            if (aa == z.name) {
                J.type = 'email';
                var ac = J.email = {},
        ...
        if (D) {
            ...
            J.adv = {};
            if (!N) {
                var O = D.getAttribute('_cke_pa_onclick') || D.getAttribute('onclick'),
                P = O && O.match(n);
                if (P) {
                    J.target.type = 'popup';
                    J.target.name = P[1];
        ...
        var U = C.document.getElementsByTag('img'),
        V = new CKEDITOR.dom.nodeList(C.document.$.anchors),
        W = J.anchors = [];
    ...
    return {
        ...
        contents: [{
            ...
            },
            {
                ...
                children: [{
                    ...
                    },
                    {
                        ...
                        onKeyUp: function() {
                            ...
                            var C = H.getDialog().getContentElement('info', 'protocol'),
                            D = H.getValue(),
                            E = /^(http|https|ftp|news):\/\/(?=.)/gi,
                            F = /^((javascript:)|[#\/\.\?])/gi,
                            G = E.exec(D);
                            if (G) {
                ...
                },
                {
                    ...
                    'default': '_blank',
                    style: 'width : 100%;',
                    items: [[a.lang.common.notSet, 'notSet'], [a.lang.common.targetNew, '_blank'], [a.lang.common.targetSelf, '_self']],
                    onChange: c,
                    setup: function(C) {
            ...
            },
            {
                ...
                children: [{
                    ...
                    setup: function(C) {
                        if (C.anchors.length > 0) this.getElement().show();
        ...
        onOk: function() {
            ...
            F = this,
            G = this.getParentEditor();
            this.commitContent(E);
            switch (E.type || 'url') {
            case 'url':
                var H = E.url && E.url.protocol != undefined ? E.url.protocol: 'http://',
                I = E.url && E.url.url || '';
                C._cke_saved_href = I.indexOf('/') === 0 ? I: H + I;
                break;
            case 'anchor':
                var J = E.anchor && E.anchor.name,
                K = E.anchor && E.anchor.id;
            ...
            case 'email':
                ...
                case '':
                case 'encode':
                    var O = encodeURIComponent(M.subject || ''),
                    P = encodeURIComponent(M.body || ''),
                    Q = [];
                    O && Q.push('subject=' + O);
                    P && Q.push('body=' + P);
                    Q = Q.length ? '?' + Q.join('&') : '';
            ...
            if (E.target) if (E.target.type == 'popup') {
                var S = ["window.open(this.href, '", E.target.name || '', "', '"],
                T = ['resizable', 'status', 'location', 'toolbar', 'menubar', 'fullscreen', 'scrollbars', 'dependent'],
                U = T.length,
                V = function(ah) {
                ...
                if (E.target.type != 'notSet' && E.target.name) C.target = E.target.name;
                else D.push('target');
            ...
            if (!this._.selectedElement) {
                ...
                if (Z.length == 1 && Z[0].collapsed) {
                    var aa = new CKEDITOR.dom.text(E.type == 'email' ? E.email.address: C._cke_saved_href, G.document);
                    Z[0].insertNode(aa);
                ...
                var ad = this._.selectedElement,
                ae = ad.getAttribute('_cke_saved_href'),
                af = ad.getHtml();
                ...
                if (ae == af || E.type == 'email' && af.indexOf('@') != -1) ad.setHtml(E.type == 'email' ? E.email.address: C._cke_saved_href);
                if (ad.getAttribute('name')) ad.addClass('cke_anchor');
                else ad.removeClass('cke_anchor');
        ...
        onFocus: function() {
            var C = this.getContentElement('info', 'linkType'),
            D;
            if (C && C.getValue() == 'url') {
